...
(function ($) {
    ...
    $.fn.imageUploader = function (options) {
        ...
        let defaults = {
            ...
            label: 'De click aquí o arrastre a este cuadro la(s) imagen(es) que desea subir',
        ...
        let createContainer = function () {
            ...
            let $uploadedContainer = $('<div>', {class: 'uploaded'}).appendTo($container),
                ...
                $i = $('<i>', {class: 'ti-cloud-up'}).appendTo($textContainer),
        ...
        let createImg = function (src, id, preloaded) {
            ...
            let $container = $('<div>', {class: 'uploaded-image'}),
                ...
                $i = $('<i>', {class: 'ti-close'}).appendTo($button);
        ...
        let validateExtension = function (file) {
            ...
            if (plugin.settings.extensions.indexOf(file.name.replace(new RegExp('^.*\\.'), '.')) < 0) {
                alert(`La extensión de "${file.name}" no forma parte de las extensiones válidas: "${plugin.settings.extensions.join('", "')}"`);
        ...
        let validateMaxSize = function (file) {
            ...
            if (file.size > plugin.settings.maxSize) {
                alert(`El archivo "${file.name}" excede el máximo tamaño permitido de ${plugin.settings.maxSize / 1024 / 1024}Mb`);
        ...
        let validateMaxFiles = function (index, file) {
            ...
            if ((index + dataTransfer.items.length + plugin.settings.preloaded.length) >= plugin.settings.maxFiles) {
                alert(`"${file.name}" no pudo agregarse porque se permite un máximo de ${plugin.settings.maxFiles} archivo(s)`);
